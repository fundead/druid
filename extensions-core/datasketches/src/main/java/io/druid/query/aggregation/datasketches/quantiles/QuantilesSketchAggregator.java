...
public class QuantilesSketchAggregator implements Aggregator
{
  ...
  private Union quantilesSketchUnion;
  ...
  static void updateQuantilesSketch(Union quantilesSketchUnion, Object update)
  {
    if (update == null) {
      return;
    }
    ...
    if (update instanceof QuantilesSketch) {
      if (!((QuantilesSketch) update).isEmpty()) {
        quantilesSketchUnion.update((QuantilesSketch) update);
      }
