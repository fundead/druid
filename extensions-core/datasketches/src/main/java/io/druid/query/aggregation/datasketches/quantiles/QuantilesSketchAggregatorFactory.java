...
public class QuantilesSketchAggregatorFactory extends AggregatorFactory
{
  ...
  {
    if (lhs == null || ((QuantilesSketch) lhs).isEmpty()) {
      return rhs;
    }

    if (rhs == null || ((QuantilesSketch) rhs).isEmpty()) {
      return lhs;
    }

    Union union = QuantilesSketchUtils.buildUnion(size);
    union.update((QuantilesSketch) lhs);
    union.update((QuantilesSketch) rhs);
